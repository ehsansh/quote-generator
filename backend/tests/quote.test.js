...
describe('Quote API', () => {
  ...
  test('GET /api/quote should handle external API error', async () => {
    ...
    expect(response.headers['content-type']).toMatch(/json/);
    expect(response.body).toHaveProperty('error', 'Failed to fetch quote from external API: Bad Request');
    expect(mockFetch).toHaveBeenCalledWith('https://dummyjson.com/quotes/random');
  });
  ...
  test('GET /api/quote should handle internal server error', async () => {
    ...
    expect(response.headers['content-type']).toMatch(/json/);
    expect(response.body).toHaveProperty('error', 'Internal Server Error while fetching quote.');
    expect(mockFetch).toHaveBeenCalledWith('https://dummyjson.com/quotes/random');
  });
